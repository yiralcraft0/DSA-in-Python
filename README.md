# 📘 DSA In Python

A personal repository where I implement **Data Structures and Algorithms (DSA)** from scratch using Python.  
This project is part of my journey to deeply understand how data structures work internally instead of only using built-in Python features.

---

## 🚀 About This Repository

This repo contains my own implementations of core data structures such as:

- Linked Lists
- Stacks
- Queues
- Trees (coming soon)
- Graphs (coming soon)
- Sorting Algorithms
- Searching Algorithms

Each implementation is written from scratch to understand:
- How data structures work internally
- Time and space complexity
- Pointer/reference manipulation
- Real-world problem solving

---

## 🧠 What I Learn From This

This repository helps me improve:

- Problem-solving skills 🧩
- Algorithmic thinking ⚙️
- Python programming fundamentals 🐍
- Data structure design concepts 📊
- Code optimization techniques 🚀

---

## 📌 Linked List Example

```python
from linked_list import LinkedList, Node

ll = LinkedList()

ll.append(20)
ll.append(30)

ll.prepend(5)

print(ll)
